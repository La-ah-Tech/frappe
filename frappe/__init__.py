...
It is the framework which powers LaahTech-EM. It is pretty generic and can
...
def has_permission(
    ...
    *,
    ...
    if throw and not out:
        document_label = (
            f"{_(doctype)} {doc if isinstance(doc, str) else doc.name}"
            if doc
            else _(doctype)
        )
...
@overload
def get_doc(
    doctype: str, name: str, /, *, for_update: bool | None = None
) -> "Document":
    ...
    for doctype in doctype_list:
        if not args.property_type:
            args.property_type = (
                db.get_value(
                    "DocField",
                    {"parent": doctype, "fieldname": args.fieldname},
                    "fieldtype",
                )
                or "Data"
            )
...
def attach_print(
    ...
    with print_language(lang or local.lang):
        ...
        if cint(print_settings.send_print_as_pdf):
            ...
            ext = ".html"
            content = html or scrub_urls(get_print(doctype, name, **kwargs)).encode(
                "utf-8"
            )
...
def get_desk_link(doctype, name):
    ...
    return html.format(
        doctype=doctype, name=name, doctype_local=_(doctype), title_local=_(title)
    )
