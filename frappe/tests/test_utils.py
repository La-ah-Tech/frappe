...
class TestAppParser(MockedRequestTestCase):
    def test_app_name_parser(self):
        ...
        self.assertEqual(
            "healthcare", parse_app_name("https://github.com/La-ah-Tech/healthcare.git")
        )
        self.assertEqual(
...
class TestTBSanitization(FrappeTestCase):
    def test_traceback_sanitzation(self):
        try:
            ...
            args = frappe._dict(
                {"password": "42", "pwd": password, "safe": "safe_value"}
            )  # noqa: F841
            raise Exception
        except Exception:
...
class TestChangeLog(FrappeTestCase):
    ...
    def test_parse_github_url(self):
        ...
        owner, repo = parse_github_url("https://github.com/La-ah-Tech/erpnext.git")
        self.assertEqual(owner, "frappe")
        self.assertEqual(repo, "erpnext")

        owner, repo = parse_github_url("https://github.com/La-ah-Tech/erpnext")
        self.assertEqual(owner, "frappe")
        self.assertEqual(repo, "erpnext")
