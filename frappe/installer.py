...
def _new_site(
    ...
    with filelock("bench_new_site", timeout=1):
        ...
        for app in apps_to_install:
            ...
            install_app(
                app, verbose=verbose, set_as_patched=not source_sql, force=False
            )
...
def find_org(org_repo: str) -> tuple[str, str]:
    ...
    find_org()
    ref -> https://github.com/La-ah-Tech/bench/blob/develop/bench/utils/__init__.py#L390
...
def fetch_details_from_tag(_tag: str) -> tuple[str, str, str]:
    ...
    ref -> https://github.com/La-ah-Tech/bench/blob/develop/bench/utils/__init__.py#L403
...
def parse_app_name(name: str) -> str:
    ...
    ref -> https://github.com/La-ah-Tech/bench/blob/develop/bench/app.py#L114
    ...
    for module_name in modules:
        ...
        for doctype in frappe.get_all(
            ...
            if not dry_run:
                if doctype.issingle:
                    frappe.delete_doc(
                        "DocType", doctype.name, ignore_on_trash=True, force=True
                    )
                else:
        ...
        if not dry_run:
            frappe.delete_doc(
                "Module Def", module_name, ignore_on_trash=True, force=True
            )
...
def _delete_linked_documents(
    ...
    for doctype, fieldname in doctype_linkfield_map.items():
        for record in frappe.get_all(
            doctype, filters={fieldname: module_name}, pluck="name"
        ):
    ...
    try:
        if file_path.endswith(".tar"):
            subprocess.check_output(
                ["tar", "xvf", tar_path, "--strip", "2"], cwd=abs_site_path
            )
        elif file_path.endswith(".tgz"):
            subprocess.check_output(
                ["tar", "zxvf", tar_path, "--strip", "2"], cwd=abs_site_path
            )
    except Exception:
        raise
    finally:
